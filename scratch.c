...
typedef struct { char __void__; } SYSTEM_VOID_T;
#define SYSTEM_VOID SYSTEM_VOID_T __SYSTEM_VOID_T__void__
...
#define system_fn_kind_(FN) \
    _Generic((FN), \
        void (*)(void): SYSTEM_FN__KIND_COUNT, \
        SystemFn_Void: SYSTEM_FN__VOID, \
        SystemFn_AppControl: SYSTEM_FN__APP_CONTROL \
    )
...
void svv(SYSTEM_VOID) {
    printf("svv\n");
}

void sa(AppControl* app_control) {
    printf("sa\n");
}
...
int main() {
    ...
    int a[] = {
        __builtin_types_compatible_p(__typeof__(&(sv)), SystemFn_Void),
        __builtin_types_compatible_p(__typeof__(&(sv)), SystemFn_AppControl),
        __builtin_types_compatible_p(__typeof__(&(sa)), SystemFn_Void),
        __builtin_types_compatible_p(__typeof__(&(sa)), SystemFn_AppControl),
        __builtin_types_compatible_p(__typeof__(&(si)), SystemFn_AppControl),

        (
            typeof_equals(&((sa)), SystemFn_AppControl) ? SYSTEM_FN__APP_CONTROL
            : typeof_equals(&((sa)), SystemFn_Void) ? SYSTEM_FN__VOID
            : SYSTEM_FN__KIND_COUNT
        )
    };
    ...
    // }
    // {
    //     SystemFn_AppControl f1 = sv;
    //     SystemFn_AppControl f2 = sa;
    //     SystemFn_AppControl f3 = si;
    ...
    // }
    // {
    //     SystemFn_AppControl_2 f1 = sv;
    //     SystemFn_AppControl_2 f2 = sa;
    //     SystemFn_AppControl_2 f3 = si;
    ...
    // SystemFnKind k1 = system_fn_kind(sv);
    // SystemFnKind k2 = system_fn_kind(sa);

    SystemFn s1 = fn_into_system(sv);
    SystemFn s2 = fn_into_system(svv);
    SystemFn s3 = fn_into_system(sa);
    // SystemFn s4 = fn_into_system(si);

    (s1.fn_void)((SYSTEM_VOID_T) {0});
    printf("kind: %d\n", s1.kind);
    (s2.fn_void)((SYSTEM_VOID_T) {0});
    printf("kind: %d\n", s2.kind);
    (s3.fn_app_control)(NULL);
    printf("kind: %d\n", s3.kind);
    // (s3.fn_void)();
